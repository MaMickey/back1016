...
var connection = new Connection(config);  
          ...
          if(err){
           ...
           console.log('here2');
             ...
             if(rows.length > 0 ){
               ...
               withPrice(level,jsonArray[0].CATLEVEL)
             }
          }
       })
       request.on('row',function(columns){})
  ...
  router.post('/detail/:id',function(req,res){
      ...
      request = new Request(`select StockExtension.CustomLabel,[D WEIGHT],StockExtension.SoldPerday, StockExtension.WebTitle,${finalPrice},StockExtension.WebDetails,stock.Stock-StockExtension.StockFro as Stock,StockExtension.CATPrimary,StockExtension.CATSub, StockExtension.ImageURL1, StockExtension.ImageURL2, StockExtension.ImageURL3, StockExtension.ImageURL4, StockExtension.ImageURL5, StockExtension.ImageURL6, StockExtension.ImageURL7, StockExtension.ImageURL8
      from (StockExtension inner join stock on stock.Code = StockExtension.CustomLabel)
      where StockExtension.CustomLabel='${id}' and WebOn = 'Y' and (stock.Stock-StockExtension.StockFro) > StockExtension.StockThreshold and StockExtension.XinWebPrice > 0
  ...
  router.post('/postage/:postcode',function(req,res){
    ...
    request = new Request(`select top 1 HunterBasicCharge,HunterMinCharge,HunterRate   from HBAllCouriers where PostCode = ${postcode}`, function(err,rowCounts,rows) { 
        ...
        if(err){console.log(err);}
          ...
          let postPrice = jsonArray[0].HunterBasicCharge + jsonArray[0].HunterRate *totalWeight
          if(postPrice > jsonArray[0].HunterMinCharge){
            res.send({postage:postPrice.toFixed(2),finalPrice:(postPrice+tempPrice).toFixed(2)})
          }
          else{
            res.send({postage:jsonArray[0].HunterMinCharge,finalPrice:jsonArray[0].HunterMinCharge+tempPrice})
          }
        } 
     })
