...
router.post('/getcart/', function(req, res) {
    ...
    for(let q =0;q<data.length;q++){
        if(data.length===1){
            label = label + `StockExtension.CustomLabel = '${data[q].cart}' `
        }
        else{
            if(q===data.length-1){
                label = label + `StockExtension.CustomLabel = '${data[q].cart}' `
                ...
                label = label + `StockExtension.CustomLabel = '${data[q].cart}' or `
            }
        }  
    }
    ...
    withPrice = (level)=>{
        ...
        request = new Request(`select [D WEIGHT], stock.Stock-StockExtension.StockFro as Stock,StockExtension.CustomLabel,StockExtension.WebTitle, StockExtension.CATPrimary,StockExtension.SoldPerday, StockExtension.CATSub,${finalPrice}
