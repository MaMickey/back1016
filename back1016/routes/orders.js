...
router.post('/orderList',function(req,res){
    ...
    withID = (IDArr)=>{
      var request = new Request(`select * from WebOrderhead where WebCustomerID = '${IDArr[0].WebCustomerID}' order by OrderDateTime desc`, function(err,rowCounts,rows) {  
        if (err) {  
            console.log(err);
          } 
        else {
    ...
    sendOrderSuccToCustomer =(jsonArray, userEmail)=>{
      ...
      var transporter = nodemailer.createTransport(smtpTransport({
        ...
        var mailOptions = {
          ...
          <td class="Email">${orderInfo.OrderDateTime}</td>
          </tr>
          <tr>
          <td class="Email">
